...
def test_get_pdf_url_march_2026():
    ...
    assert get_pdf_url(dt) == "https://meralcomain.s3.ap-southeast-1.amazonaws.com/2026-03/03-2026_residential_bills.pdf"


def test_get_pdf_url_february_2026():
    dt = datetime(2026, 2, 1)
    assert get_pdf_url(dt) == "https://meralcomain.s3.ap-southeast-1.amazonaws.com/2026-02/02-2026_residential_bills.pdf"
...
class TestParseResidentialBills:
    def test_returns_all_consumption_levels(self):
        result = parse_residential_bills(_load_rows(FIXTURE_BILLS_MAR))
        kwh_values = [r["kwh"] for r in result]
        assert kwh_values == [50, 70, 100, 200, 300, 400, 500, 600, 700, 800, 900, 1000, 1500, 3000, 5000]

    def test_200_kwh_matches_published_rate_mar(self):
        result = parse_residential_bills(_load_rows(FIXTURE_BILLS_MAR))
        row_200 = next(r for r in result if r["kwh"] == 200)
        assert row_200["rate"] == 13.8161

    def test_200_kwh_matches_published_rate_feb(self):
        result = parse_residential_bills(_load_rows(FIXTURE_BILLS_FEB))
        row_200 = next(r for r in result if r["kwh"] == 200)
        assert row_200["rate"] == 13.1734

    def test_each_entry_has_kwh_and_rate_only(self):
        result = parse_residential_bills(_load_rows(FIXTURE_BILLS_MAR))
        for entry in result:
    ...
    def test_rows_without_non_lifeline_marker_returns_empty(self):
        rows = [["50", "foo", "bar"], ["100", "baz", "qux"]]
...
class TestComputeRateChanges:
    ...
    def test_positive_change_mar_vs_feb(self):
        mar = parse_residential_bills(_load_rows(FIXTURE_BILLS_MAR))
        feb = parse_residential_bills(_load_rows(FIXTURE_BILLS_FEB))
        changed = compute_rate_changes(mar, feb)
    ...
    def test_null_changes_when_no_previous(self):
        mar = parse_residential_bills(_load_rows(FIXTURE_BILLS_MAR))
    ...
    def test_null_changes_when_empty_previous(self):
        mar = parse_residential_bills(_load_rows(FIXTURE_BILLS_MAR))
...
class TestExtractBillingDate:
    def test_extracts_date_from_mar_fixture(self):
        rows = _load_rows(FIXTURE_BILLS_MAR)
        assert _extract_billing_date(rows) == "03/2026"

    def test_extracts_date_from_feb_fixture(self):
    ...
    def test_returns_none_when_no_month_found(self):
...
class TestDownloadPdf:
    ...
    def test_failure_returns_none(self, tmp_path, monkeypatch):
        monkeypatch.setattr("src.parser.PDF_CACHE_DIR", str(tmp_path))
        with patch("src.parser.urllib.request.urlopen", side_effect=Exception("network error")):
            result = download_pdf("https://example.com/test.pdf")
    ...
    def test_uses_cache_on_second_call(self, tmp_path, monkeypatch):
        ...
        with patch("src.parser.urllib.request.urlopen", return_value=mock_response) as mock_urlopen:
...
class TestCleanupOldPdfs:
    def test_removes_old_pdfs(self, tmp_path, monkeypatch):
        monkeypatch.setattr("src.parser.PDF_CACHE_DIR", str(tmp_path))
        ...
        assert remaining == ["02-2026_residential_bills.pdf", "03-2026_residential_bills.pdf"]
...
class TestGetMeralcoRates:
    ...
    def test_success_with_real_pdfs(self, mock_download):
        ...
        entry_200 = next(e for e in result["data"] if e["kwh"] == 200)
        assert entry_200["rate"] == 13.8161
        assert entry_200["rate_change"] == round(13.8161 - 13.1734, 4)
        assert entry_200["trend"] == "up"
